...
if (Test-Path $packageJson) {
    ...
    try {
        ...
        if ($currentDev -eq "tsx server.ts") {
            ...
            if (-not ($pkg.scripts.PSObject.Properties.Name -contains "dev_legacy")) {
                $pkg.scripts | Add-Member -NotePropertyName "dev_legacy" -NotePropertyValue "tsx server.ts"
            } else {
                $pkg.scripts.dev_legacy = "tsx server.ts"
            }
            $pkg.scripts.dev = "vite"
        } elseif (-not $currentDev) {
            Log-Line "No dev script found. Creating dev -> vite."
            if (-not ($pkg.scripts.PSObject.Properties.Name -contains "dev")) {
                $pkg.scripts | Add-Member -NotePropertyName "dev" -NotePropertyValue "vite"
            } else {
                $pkg.scripts.dev = "vite"
            }
        } else {
        ...
        if (-not ($pkg.scripts.PSObject.Properties.Name -contains "build")) {
            $pkg.scripts | Add-Member -NotePropertyName "build" -NotePropertyValue "vite build"
            Log-Line "Added build script."
        }

        if (-not ($pkg.scripts.PSObject.Properties.Name -contains "preview")) {
            $pkg.scripts | Add-Member -NotePropertyName "preview" -NotePropertyValue "vite preview"
...
- package.json backup was created before editing
...
Write-Host ""
Write-Host "DONE"
Write-Host ("Run next: powershell -ExecutionPolicy Bypass -File .\scripts\run_system_fixed.ps1 -ProjectRoot `"" + $root + "`"")
