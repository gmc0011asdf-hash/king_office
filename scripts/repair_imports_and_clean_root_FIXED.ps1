...
$script:LogFile = Join-Path $fixDir ("repair_imports_and_clean_root_FIXED_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".txt")
...
foreach ($file in $pageFiles) {
    Replace-InFile -Path $file.FullName -Pairs $pagePairs
}
...
foreach ($file in $apiFiles) {
    Replace-InFile -Path $file.FullName -Pairs $apiPairs
}
...
Replace-InFile -Path $layoutPath -Pairs $layoutPairs
...
$scriptCandidates = @(
    ...
    "repair_imports_and_clean_root.ps1",
    "repair_imports_and_clean_root_FIXED.ps1"
)
